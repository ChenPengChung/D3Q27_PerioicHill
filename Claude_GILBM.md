...
壁面（山丘表面）精確落在 k=2 格線上：z[j\*NZ6+2] = H(y[j])
...
### 1.4 網格結構（Option B：k=2 為壁面計算點）

```
k 索引    角色               z 座標                  是否計算
─────────────────────────────────────────────────────────────
k=0,1    ghost layer        未定義                   否
k=2      壁面計算點          H(y)（山丘表面）          是 ← C-E BC
k=3      第一內部計算點      H(y) + minSize/2         是
...      內部計算點          tanh 拉伸                是
k=NZ6-4  倒數第二內部點                               是
k=NZ6-3  頂壁計算點          LZ                       是 ← C-E BC
k=NZ6-2, NZ6-1  ghost layer                          否
```

Kernel guard: `if (k <= 1 || k >= NZ6-2) return;`
...
對壁面節點（k=2 底壁, k=NZ6-3 頂壁），某些方向需 Chapman-Enskog BC：
```
...
bk: clamp to [2, NZ6-9]        （z 壁面，k=2 到 k=NZ6-3）
```
...
壁面速度梯度（二階單側差分，u[wall] = 0）：
```
∂u_i/∂k|_{k=2} = (4·u_i[k=3] - u_i[k=4]) / 2
```

壁面密度：ρ_wall ≈ ρ[k=3]（Imamura §3.2：壁面法向壓力梯度為零）
...
dz_dj 在 k=2 只涉及 j±1 鄰居（同一 k=2 層）：
```
dz_dj|_{k=2} = (H(y[j+1]) - H(y[j-1])) / 2 = dH/dy
```
**不需要 k 方向鄰居**，精度 ≥ 內部點。
